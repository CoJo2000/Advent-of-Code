 ...
 *
	...
	sure why the water stopped; however, he can show you how to get to the water source to 
...
public class Day2 {
	...
	public static int part1() {
		...
		try {	
			// Find/Open File
			String fLoc = System.getProperty("user.dir") + "\\src\\Y2k23\\Inputs\\Day2Part1.txt";
			...
			while (myReader.hasNextLine()) {
				...
				// Convert Line into parts of the Game Number, and individual grabs. 
				String[] grabs = game.split("[:]|[;]|[\n]");
				// Is the game valid, ie, the values are less than their color max
				boolean validGame = true;
				
				// Loop for Checking individual grabs. 
				for (int x=1; x<grabs.length; x++) {
					// Convert grabs into individual cube colors. 
					String curGrab = grabs[x].strip();
					String[] individualCubes = curGrab.split(",");
					
					// Check if each color does not go over the color max. 
					for (int i=0; i<individualCubes.length; i++) {
						if (individualCubes[i].contains("red")) {
							int curRed = Integer.parseInt(individualCubes[i].replace(" red", "").strip());
							if (curRed > redMax) { validGame = false; }
						} else if (individualCubes[i].contains("blue")) {
							int curBlue = Integer.parseInt(individualCubes[i].replace(" blue", "").strip());
							if (curBlue > blueMax) { validGame = false; }
						} else if (individualCubes[i].contains("green")) {
							int curGreen = Integer.parseInt(individualCubes[i].replace(" green", "").strip());
							if (curGreen > greenMax) { validGame = false; }
						}
					}
				}
				
				// If the game is valid, add the game's ID number to the total
				if (validGame) {
					total += Integer.parseInt(grabs[0].replace("Game ", "").strip());
				}
			}
	...
	public static int part2() {
		try {	
			// Find/Open File
			String fLoc = System.getProperty("user.dir") + "\\src\\Y2k23\\Inputs\\Day2Part2.txt";
			File myObj = new File(fLoc);
			// Scan File
			Scanner myReader = new Scanner(myObj);
			int total = 0;
			while (myReader.hasNextLine()) {
				...
				for (int x=1; x<grabs.length; x++) {
					...
					// Find the max for each color in a game's grabs. 
					for (int i=0; i<individualCubes.length; i++) {
						if (individualCubes[i].contains("red")) {
							int curRed = Integer.parseInt(individualCubes[i].replace(" red", "").strip());
							if (curRed > redMax) { redMax = curRed; }
						} else if (individualCubes[i].contains("blue")) {
							int curBlue = Integer.parseInt(individualCubes[i].replace(" blue", "").strip());
							if (curBlue > blueMax) { blueMax = curBlue; }
						} else if (individualCubes[i].contains("green")) {
							int curGreen = Integer.parseInt(individualCubes[i].replace(" green", "").strip());
							if (curGreen > greenMax) { greenMax = curGreen; }
						}
					}
				}
				// Add the multiplied values of colors to the total
